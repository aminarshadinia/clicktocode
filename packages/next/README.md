...
`<ClickToCode />` defaults to OpenCode on <kbd>Alt</kbd>, plus the clipboard picker on <kbd>⌘C</kbd> (Mac) / <kbd>Ctrl+C</kbd> (Windows/Linux). Needs the CLI: `npm i -g opencode-ai@latest && opencode auth login`.
